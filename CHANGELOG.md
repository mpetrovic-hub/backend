...
- [DB Retention Archive Health] Opened Health PRAGMA checks through an immutable read-only SQLite URI so `diagnose` and scheduled checks no longer create WAL/SHM sidecars. A pre-existing non-empty WAL now fails closed before the PRAGMA instead of letting an immutable snapshot ignore uncheckpointed committed rows.
...
- [DB Retention Archive Health] Closed the confirmed-corruption handoff race: if the child cannot persist the filesystem sentinel, it keeps the generation lock while the parent persists the Corruption Incident, then releases only after acknowledgement. Timeout handling never terminates a started child, even before readiness is observed; if both corruption gates are temporarily unavailable, the child remains locked and retries the sentinel until a durable gate exists. The child rechecks filesystem gates under its acquired generation lock before opening SQLite; scheduled checks defer behind them, while explicitly confirmed unblock verification can still run its required read-only integrity check behind a well-formed gate. After a healthy child result, the controller rechecks central gate state before resolving Availability. Corruption lifecycle idempotency survives repair and later identical re-detection, and sentinel/Incident reconciliation rechecks both sources under the generation lock. Persisting a gate from completed PRAGMA evidence now captures either pre-existing corruption source and performs the same locked recheck, so stale check evidence cannot reopen a concurrently completed sentinel- or Incident-only recovery. Failed Availability resolution is retried only from a definitive corruption sentinel or Incident, never from a replacement-transition marker; unreadable or malformed gate state, including malformed replacement-transition markers, raises Availability and exits 2. `diagnose`/`unblock` now reject path-like input instead of stripping it to a basename. Unblock verification itself now persists a gate on confirmed corruption, preserves technical child errors with exit 2, and retains expected corruption or timeout blocks as exit 1. Manual replacement requires and revalidates the source corruption gate under A's lock, rejects a B with its own Corruption gate, accepts only the distinct generation cleanup currently selects (including current-year B after rollover), keeps B durably blocked by a distinct non-corruption transition marker until A's Corruption Incident resolves, prevents stale workers or audit writes from resurrecting terminalized runs, and reports durable recovery failures with exit 2. Operational Event cleanup conditionally refreshes every still-open Corruption Incident before old-row deletion, never reopens a concurrently recovered correlation, and fails closed without deleting events if a required fallback gate cannot be renewed.
- [DB Retention Archive Health] Serialized the final `archive_lock_active` Availability decision with its lifecycle transition. A one-shot check now probes the generation lock once immediately before persisting: a still-active lock records the failure in lifecycle order, while an already-released lock suppresses only the stale Incident transition without waiting, retrying, or rerunning SQLite.
- [DB Retention Archive Health] Added bounded supersession ordering without a new state store: an older non-definitive result cannot reopen Availability after a later persisted definitive recovery, while a timeout without such recovery still opens normally. Child-reported Corruption gates now survive Parent handoff unless a later audited manual `unblock` proves recovery; missing gates without that proof are restored fail-closed, and sentinel-only unblock stops if its Incident audit cannot persist.
- [DB Retention Archive Health] Completed Availability supersession ordering in both directions: an older definitive result can no longer resolve a failure from a later-started check. The comparison remains inside the existing short lifecycle lock and adds no retry, state store, or routine success event.

2026-07-29:
- [DB Retention Archive Health] Moved archive-wide SQLite checks out of retention WP-Cron into a supervised external WP-CLI controller with shared OS locks, atomic schedule state, daily/annual checks, timeout cleanup, Operational Incidents, quarantine generations, and deterministic recovery runs. MySQL deletion now requires an exact persisted SQLite receipt and reconciles crash windows before advancing audit cursors. If receipt rows commit but later SQLite batch finalization fails, the original cleanup run remains resumable and re-verifies the durable receipt before recording archive counts or deleting MySQL rows; the same re-read now also covers a one-time receipt repair whose rows committed before its finalization failed. A receipt that remains invalid after its one repair attempt now leaves the same frozen cleanup run nonterminally blocked, preventing normal cron from creating or retrying an overlapping scope until controlled recovery; a transient central Incident-write failure is retried idempotently without repeating repair or delete work. Open-run and Operational Event correlation lookup errors also fail closed instead of being treated as an empty queue or no open Incident. Daily checks follow an open cleanup run's frozen archive path across calendar-year rollover, preserve an incomplete target across Berlin date changes, count target-resolution failures, WordPress loader exceptions, scheduled service exceptions, and exactly-once reconciled non-blocking controller deferrals toward the incident threshold, fail closed whenever a persisted daily target disappears, report only actual raised/repeated/resolved Incident lifecycle actions, leave lookup Incidents open when a recovered lookup still produces `no_work`, reconcile unrecorded predecessor quarantine markers only after the critical corruption Incident and matching incomplete-Incident recovery are durable, and classify definitive SQLite format/corruption exceptions for quarantine. Confirmed corruption also persists a generation-specific worker write block beside the held lock before independently attempting the Incident and quarantine marker, so a concurrent read-only diagnostic or failure of either later persistence step cannot reopen archive/delete work. Direct daily corruption now acknowledges its quarantine marker only after both lookup-scoped and archive-scoped incomplete-Incident recovery are durable; if its final controller-state write failed, a later marker recovery retains the original Berlin detection date so it cannot consume the current day's active-generation check. A normal cleanup with no prior open run remains frozen on an unacknowledged quarantined predecessor and cannot select or write its successor until controller acknowledgement is durable. Missing files from a frozen annual snapshot remain pending and fail visibly instead of being recorded as skipped; unfinished annual snapshots now survive calendar-year rollover, and a later complete annual result, including immediate acknowledgement after reconciliation of an already-persisted annual quarantine marker, resolves the archive's earlier incomplete-check incident without consuming or overwriting the next Berlin day's active-archive check. Annual entries remain retryable until required Incident recovery persists, while strict campaign audit state records start/completion timestamps, a stable sorted relative snapshot, and each file's status, integrity mode, and last definitive result. Each SQLite child now retains its own shared generation lock until it exits, so a surviving timed-out child cannot overlap a writer after the controller releases its lock. Timed-out children never enter blocking `proc_close()` while still running, transient successor-path/count/transaction failures reschedule quarantine recovery instead of terminally failing it, and the versioned CLI result contract normalizes public check names plus nullable archive and incident fields. Status queries apply retention event-type filters before bounded repository reads, propagate query failures, and validate frozen archive paths without creating an absent archive root, so unrelated Incident volume or unreadable Incident storage cannot hide relevant health state. Lookup incidents remain open until target resolution is followed by a completed daily result. Preflight now isolates its production-shaped scratch SQLite file in a hidden non-discoverable subdirectory, preventing annual snapshot races. When a deterministic corruption successor has zero rows, its unresolved recovery context now survives archive-year rollover until the first later non-empty verified batch resolves and durably marks it; successors keep the quarantined archive's year across rollover, chained quarantines carry every unresolved predecessor context forward until the next qualifying batch, and acknowledged no-open-run quarantine markers remain recovery evidence across archive years. Daily and annual corruption now acknowledge the quarantine marker only after the controller state and required incomplete-Incident recovery are durable, and both the main bootstrap path and standalone fallback persist controller-lock deferrals while auditing scheduled bootstrap failures; both reconcile older receipts before advancing a new bootstrap-failure attempt window.
