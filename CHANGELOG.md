...
- [DB Retention Archive Health] Closed the confirmed-corruption handoff race: if the child cannot persist the filesystem sentinel, it keeps the generation lock while the parent persists the Corruption Incident, then releases only after acknowledgement. Failed Availability resolution is retried from the durable corruption gate, and `diagnose`/`unblock` now reject path-like input instead of stripping it to a basename. Unblock verification itself now persists a gate on confirmed corruption. Manual replacement requires an existing source corruption gate, accepts only the distinct generation cleanup currently selects (including current-year B after rollover), keeps B durably blocked by a distinct non-corruption transition marker until A's Corruption Incident resolves, and prevents stale workers or audit writes from resurrecting terminalized runs.
