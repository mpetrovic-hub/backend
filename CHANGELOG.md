...
2026-07-29:
- [DB Retention Archive Health] Moved archive-wide SQLite checks out of retention WP-Cron into a supervised external WP-CLI controller with shared OS locks, atomic schedule state, daily/annual checks, timeout cleanup, Operational Incidents, quarantine generations, and deterministic recovery runs. MySQL deletion now requires an exact persisted SQLite receipt and reconciles crash windows before advancing audit cursors. Daily checks follow an open cleanup run's frozen archive path across calendar-year rollover, fail closed when a progressed archive disappears, and classify definitive SQLite format/corruption exceptions for quarantine.
