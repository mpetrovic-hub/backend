...
- [DB Retention Archive Health] Closed the confirmed-corruption handoff race: if the child cannot persist the filesystem sentinel, it keeps the generation lock while the parent persists the Corruption Incident, then releases only after acknowledgement. Failed Availability resolution is retried from the durable corruption gate, and `diagnose`/`unblock` now reject path-like input instead of stripping it to a basename. Manual replacement also requires an existing source corruption gate, rejects any generation cleanup would not select, keeps the verified replacement durably blocked by a distinct non-corruption transition marker until the old archive's Corruption Incident resolves successfully, and prevents stale workers or audit writes from resurrecting terminalized runs.
