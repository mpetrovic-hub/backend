...
2026-09-04:
- [NTH FR One-off SMS Variants] Replaced the uniform technical variant pool with the fixed eight-entry `fr_sms_v2` weighted allocation, including the new `download_phrase` type and no new `bare_id` traffic. Assignment and long-lived summary rows now carry `allocation_version`, delayed historical events remain in `legacy`, and event markers commit atomically with their version-aware or pre-schema legacy summary counters on required InnoDB storage. The external schema contract verifies both table engines, the column definition, and the full non-prefixed BTREE summary identity while rejecting the legacy unique index. New enrollment is default-off until the schema deployment gate passes; existing assignments retain their stored body and remain measurable when enrollment stops; and the allocation is restricted to the exact FR NTH One-off integration context.
