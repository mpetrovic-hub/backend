...
2026-09-04:
- [NTH FR One-off SMS Variants] Replaced the uniform technical variant pool with the fixed eight-entry `fr_sms_v2` weighted allocation, including the new `download_phrase` type and no new `bare_id` traffic. The NTH integration adapter owns the concrete FR scope and allocation while the shared service consumes a normalized contract. Assignment enrollment commits atomically with its initial summary increment; delayed historical events remain in `legacy`, and missing summaries are reconstructed under a bounded database-scoped hashed identity lock shared with missing-summary enrollment. Runtime writes fail closed until both tables are verified as InnoDB; ordinary enrollment and event writes avoid the advisory lock and full-cohort recounts while event markers still commit atomically with their version-aware or pre-schema counters. The external schema contract verifies both table engines, fully non-whitespace version values, the column definition, and the full non-prefixed BTREE summary identity while rejecting any reordered or renamed unique copy of the narrower legacy identity. New enrollment is default-off until the schema deployment gate passes, and existing assignments retain their stored body when enrollment stops.
...
2026-08-01:
- [DB Retention Archive Health] Moved archive-wide SQLite health checks out of retention WP-Cron into an external WP-CLI process. Retention cleanup remains archive-before-delete and resumes safely from persisted SQLite receipts after interrupted runs. (#112)
- [DB Retention Archive Health] Replaced the broad controller, campaign, quarantine, and automatic-recovery design with the lean external `check`, read-only `diagnose`, and confirmed `unblock` contract. Only a completed non-`ok` SQLite check may block retention writes; availability faults remain separate. (#114)
- [DB Retention Archive Health] Hardened the corruption and recovery handoff: health checks and cleanup share non-waiting generation locks, unavailable safety gates fail closed, and repair or replacement requires explicit, verified operator action.
