...
function kiwi_test_cron_event_key($hook, array $args = []): string
{
    return (string) $hook . (empty($args) ? '' : '|' . md5(serialize($args)));
}
...
function wp_next_scheduled($hook, $args = [])
{
    ...
    return $GLOBALS['kiwi_test_next_scheduled'][kiwi_test_cron_event_key($hook, $args)] ?? false;
}
...
function wp_schedule_single_event($timestamp, $hook, $args = []): bool
{
    ...
    $GLOBALS['kiwi_test_cron_events'][] = $event;
    $GLOBALS['kiwi_test_next_scheduled'][kiwi_test_cron_event_key($hook, $args)] = (int) $timestamp;
...
class Kiwi_Test_Retention_Sqlite_Archive_Service extends Kiwi_Retention_Sqlite_Archive_Service
{
    ...
    public function archive_primary_key_chunk(
        ...
        return [
            ...
            'archive_batch_id' => $archive_batch_id,
            'archive_db_path' => '/tmp/kiwi_retention_archive_2026.sqlite',
            'archived_rows' => count($this->archived_primary_keys),
            'archive_inserted_rows' => count($this->archived_primary_keys),
            'archive_duplicate_rows' => 0,
...
class Kiwi_Test_Wpdb_Retention_Sqlite_Archive
{
    ...
    public function get_var($statement)
    {
        ...
        if (strpos($query, 'ORDER BY id ASC') === false) {
            return null;
        }
        ...
        $cutoff = (string) ($args[0] ?? '');
        $last_id = (int) ($args[1] ?? 0);
        $target_max_primary_key = (int) ($args[2] ?? PHP_INT_MAX);
        $rows = array_values(array_filter($this->rows, static function (array $row) use ($cutoff, $last_id, $target_max_primary_key): bool {
            $id = (int) ($row['id'] ?? 0);
...
class Kiwi_Test_Retention_Cleanup_Dispatch_Service extends Kiwi_Retention_Cleanup_Service
{
    public $run_source_calls = [];
    public $run_worker_calls = [];
    public $source_results = [];
    public $worker_results = [];
    ...
    {
        $this->run_source_calls[] = [
            'source_key' => $source_key,
            'triggered_by' => $triggered_by,
        ];

        return $this->source_results[$source_key] ?? [
            'success' => true,
            'status' => 'pending',
    ...
    {
        $this->run_worker_calls[] = $source_key;

        return $this->worker_results[$source_key] ?? [
            'success' => true,
            'status' => 'partial',
...
class Kiwi_Test_Sequenced_Retention_Corruption_Safety_Gate_Coordinator
    extends Kiwi_Retention_Corruption_Safety_Gate_Coordinator
{
    public $inspection_count = 0;
    public $block_on_inspection = 0;
    ...
    public function inspect(string $archive_path, bool $reconcile = false): array
    {
        ...
        $blocked = $this->inspection_count === $this->block_on_inspection;

        return [
            'allowed' => !$blocked,
            'reason_code' => $blocked
                ? 'archive_corruption_write_blocked'
                : 'corruption_gate_clear',
...
class Kiwi_Test_Retention_Cleanup_Service extends Kiwi_Retention_Cleanup_Service
{
    ...
    public $target_max_primary_key = 0;
    ...
    protected function determine_target_max_primary_key(array $source, string $cutoff_value): int
    {
        ...
        return $this->target_max_primary_key;
    }
...
kiwi_run_test('Kiwi_Sms_Body_Variant_Service exposes the exact fr_sms_v2 allocation and renders every SMS form', function (): void {
    ...
    kiwi_assert_same('fr_sms_v2', $service->get_allocation_version(), 'Expected the weighted allocation to have a stable reporting version.');
    kiwi_assert_same($expected, $service->get_active_allocation(), 'Expected exactly the approved eight active variants and weights.');
    kiwi_assert_same(100, array_sum(array_column($service->get_active_allocation(), 'weight')), 'Expected active allocation weights to total 100 percent.');
    kiwi_assert_same(['TopJeux', 'JeuxMax', 'GameQuest'], $service->get_game_seeds(), 'Expected only approved game-word seeds to stay active.');
    kiwi_assert_same(['BonusJeux', 'JouerPlus', 'AccederJeux'], $service->get_cta_seeds(), 'Expected only approved CTA-phrase seeds to stay active.');
    kiwi_assert_same(false, in_array('bare_id', array_column($service->get_active_allocation(), 'variant_key'), true), 'Expected bare_id to receive no new traffic.');
...
kiwi_run_test('Kiwi_Config exposes final retention defaults for landing page sessions', function (): void {
    ...
    kiwi_assert_same(50000, $config->get_retention_worker_row_limit(), 'Expected bounded worker row limit to default to 50,000 rows.');
    ...
    kiwi_assert_same(
        ['enabled' => false, 'dry_run' => true, 'retention_days' => 21],
        $config->get_retention_source_settings('landing_handoff_events'),
        'Expected handoff retention to stay default-off and dry-run with twenty-one complete days.'
    );
});

kiwi_run_test('Kiwi_Retention_Source_Registry exposes stable session and handoff contracts', function (): void {
    global $wpdb;

    $previous_wpdb = $wpdb ?? null;
    $wpdb = (object) ['prefix' => 'wp_'];
    $registry = new Kiwi_Retention_Source_Registry();
    $sessions = $registry->get(Kiwi_Retention_Source_Registry::SOURCE_LANDING_PAGE_SESSIONS);
    $handoffs = $registry->get(Kiwi_Retention_Source_Registry::SOURCE_LANDING_HANDOFF_EVENTS);

    kiwi_assert_same(
        [
            Kiwi_Retention_Source_Registry::SOURCE_LANDING_PAGE_SESSIONS,
            Kiwi_Retention_Source_Registry::SOURCE_LANDING_HANDOFF_EVENTS,
        ],
        $registry->get_source_keys(),
        'Expected registered retention sources to keep their stable scheduler order.'
    );
    kiwi_assert_same(true, $sessions['coverage_gate_required'] ?? null, 'Expected landing sessions to remain coverage-gated.');
    kiwi_assert_same(false, $handoffs['coverage_gate_required'] ?? null, 'Expected handoff retention to opt out of summary coverage.');
    kiwi_assert_same('wp_kiwi_landing_handoff_events', $handoffs['source_table'] ?? '', 'Expected the strict handoff source table.');
    kiwi_assert_same('id', $handoffs['primary_key'] ?? '', 'Expected the handoff primary key.');
    kiwi_assert_same('created_at', $handoffs['cutoff_column'] ?? '', 'Expected the handoff cutoff column.');
    kiwi_assert_same(21, $handoffs['retention_days_default'] ?? 0, 'Expected the handoff twenty-one-day default.');
    kiwi_assert_same(7, $handoffs['retention_days_min'] ?? 0, 'Expected the handoff seven-day minimum.');
    kiwi_assert_same(
        [
            'id' => 'INTEGER',
            'created_at' => 'TEXT',
        ...
        $handoffs['archive_columns'] ?? [],
        'Expected every current handoff source column to have an exact archive mapping.'
    );
    $repository_source = file_get_contents(
        __DIR__ . '/../includes/repositories/class-landing-handoff-event-repository.php'
    );
    $schema_match = [];
    $schema_columns = [];
    if (is_string($repository_source)
        && preg_match('/CREATE TABLE\s+\{\$table_name\}\s*\((.*?)\s+PRIMARY KEY \(id\),/s', $repository_source, $schema_match) === 1
    ) {
        $column_matches = [];
        preg_match_all(
            '/^\s*([a-z][a-z0-9_]*)\s+(?:BIGINT|DATETIME|VARCHAR|TINYINT|INT|TEXT|LONGTEXT)\b/m',
            (string) ($schema_match[1] ?? ''),
            $column_matches
        );
        $schema_columns = $column_matches[1] ?? [];
    }
    kiwi_assert_same(
        $schema_columns,
        array_keys($handoffs['archive_columns'] ?? []),
        'Expected the handoff archive mapping to stay complete and ordered with the repository schema.'
    );
...
kiwi_run_test('Kiwi_Plugin schedules the retention cleanup scheduler daily cron hook once', function (): void {
    ...
    $reflection = new ReflectionClass(Kiwi_Plugin::class);
    $legacy_hook = (string) $reflection->getConstant('RETENTION_CLEANUP_LEGACY_DAILY_HOOK');
    $scheduler_hook = (string) $reflection->getConstant('RETENTION_CLEANUP_SCHEDULER_DAILY_HOOK');
    $worker_hook = (string) $reflection->getConstant('RETENTION_CLEANUP_WORKER_HOOK');
    ...
    kiwi_assert_true(isset($GLOBALS['kiwi_test_hooks'][$scheduler_hook]), 'Expected the retention scheduler cron hook to be registered.');
    kiwi_assert_true(isset($GLOBALS['kiwi_test_hooks'][$worker_hook]), 'Expected the retention worker cron hook to be registered.');
    kiwi_assert_true(in_array($legacy_hook, $GLOBALS['kiwi_test_cleared_hooks'], true), 'Expected scheduling to clear the legacy unbounded cleanup hook.');
    kiwi_assert_same(1, count(array_filter($GLOBALS['kiwi_test_cron_events'], static function (array $event) use ($scheduler_hook): bool {
        return ($event['hook'] ?? '') === $scheduler_hook;
    })), 'Expected retention scheduler cleanup to be scheduled only once.');
...
kiwi_run_test('Kiwi_Plugin dispatches source-keyed retention scheduler and worker hooks in registry order', function (): void {
    $GLOBALS['kiwi_test_cron_events'] = [];
    $GLOBALS['kiwi_test_next_scheduled'] = [];
    $service = new Kiwi_Test_Retention_Cleanup_Dispatch_Service();
    $plugin = new Kiwi_Test_Plugin_Retention_Cleanup($service);
    $reflection = new ReflectionClass(Kiwi_Plugin::class);
    $worker_hook = (string) $reflection->getConstant('RETENTION_CLEANUP_WORKER_HOOK');

    $first = $plugin->run_retention_cleanup_scheduler_daily();
    $plugin->run_retention_cleanup_scheduler_daily();
    $plugin->run_retention_cleanup_worker();
    $plugin->run_retention_cleanup_worker(Kiwi_Retention_Source_Registry::SOURCE_LANDING_HANDOFF_EVENTS);
    ...
    kiwi_assert_same(
        ...
        array_keys($first['source_results'] ?? []),
        'Expected scheduler results in stable registry order.'
    );
    kiwi_assert_same(
        [
            ['source_key' => 'landing_page_sessions', 'triggered_by' => 'cron'],
            ['source_key' => 'landing_handoff_events', 'triggered_by' => 'cron'],
            ['source_key' => 'landing_page_sessions', 'triggered_by' => 'cron'],
            ['source_key' => 'landing_handoff_events', 'triggered_by' => 'cron'],
        ],
        $service->run_source_calls,
        'Expected every daily scheduler pass to dispatch each registered source in order.'
    );
    kiwi_assert_same(
        ['landing_page_sessions', 'landing_handoff_events'],
        $service->run_worker_calls,
        'Expected an argumentless worker to remain compatible with landing sessions and an explicit handoff worker to stay isolated.'
    );
    $worker_events = array_values(array_filter(
        $GLOBALS['kiwi_test_cron_events'],
        static function (array $event) use ($worker_hook): bool {
            return ($event['hook'] ?? '') === $worker_hook;
        }
    ));
    kiwi_assert_same(2, count($worker_events), 'Expected source-key deduplication to keep exactly one pending worker per source.');
    kiwi_assert_same(
        [['landing_page_sessions'], ['landing_handoff_events']],
        array_column($worker_events, 'args'),
        'Expected every worker event to carry its source key.'
    );
});
...
kiwi_run_test('Kiwi_Retention_Sqlite_Archive_Service keeps handoff cutoff and table boundaries strict', function (): void {
    if (!class_exists('PDO') || !in_array('sqlite', PDO::getAvailableDrivers(), true)) {
        return;
    }
    ...
    $wpdb = new Kiwi_Test_Wpdb_Retention_Sqlite_Archive();
    $wpdb->rows = [
        ['id' => 301, 'created_at' => '2026-03-10 23:59:59', 'landing_key' => 'lp-old', 'session_token' => 'session-old'],
        ['id' => 302, 'created_at' => '2026-03-11 00:00:00', 'landing_key' => 'lp-edge', 'session_token' => 'session-edge'],
        ['id' => 303, 'created_at' => '2026-03-11 00:00:01', 'landing_key' => 'lp-new', 'session_token' => 'session-new'],
    ];
    $archive_root = kiwi_create_temp_directory('kiwi_handoff_retention_archive_test');
    $service = new Kiwi_Retention_Sqlite_Archive_Service(new Kiwi_Test_Retention_Sqlite_Archive_Config($archive_root));
    $source = (new Kiwi_Retention_Source_Registry())->get('landing_handoff_events');
    $pdo = null;

    try {
        $result = $service->archive_eligible_rows($source, '2026-03-11 00:00:00', 'handoff-archive-test', 1);
        $archive_db_path = (string) ($result['archive_db_path'] ?? '');
        $queries = array_column($wpdb->prepared_statements, 'query');

        kiwi_assert_same(true, $result['success'] ?? false, 'Expected handoff SQLite archive to succeed.');
        kiwi_assert_same(1, $result['archived_rows'] ?? 0, 'Expected only the handoff row strictly before the cutoff.');
        kiwi_assert_same(
            'deferred_to_external_health_runner',
            $result['archive_integrity_check'] ?? '',
            'Expected handoff archive-wide health checks to stay outside runtime.'
        );
        kiwi_assert_true(
            count(array_filter($queries, static function (string $query): bool {
                return strpos($query, 'FROM wp_kiwi_landing_handoff_events') !== false;
            })) > 0,
            'Expected handoff archive reads to stay inside the handoff source table.'
        );
        kiwi_assert_same(
            0,
            count(array_filter($queries, static function (string $query): bool {
                return strpos($query, 'wp_kiwi_landing_page_sessions') !== false;
            })),
            'Expected handoff archive reads never to touch landing sessions.'
        );

        $pdo = new PDO('sqlite:' . $archive_db_path);
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
        $archive_table = '"' . str_replace('"', '""', (string) ($source['source_table'] ?? '')) . '"';
        $column_rows = $pdo->query('PRAGMA table_info(' . $archive_table . ')')->fetchAll(PDO::FETCH_ASSOC);
        kiwi_assert_same(
            array_merge(
                ['_archive_batch_id', '_archived_at', '_source_pk'],
                array_keys($source['archive_columns'] ?? [])
            ),
            array_column($column_rows, 'name'),
            'Expected the handoff archive table to contain exactly the shared evidence and full source mapping.'
        );
        kiwi_assert_same(
            [301],
            $service->fetch_archived_primary_key_batch($source, $archive_db_path, 'handoff-archive-test', 0, 10),
            'Expected the handoff receipt to contain only the strict pre-cutoff row.'
        );
    } finally {
        $pdo = null;
        $wpdb = $previous_wpdb;
...
kiwi_run_test('Kiwi_Retention_Sqlite_Archive_Service chunk resume preserves batch row evidence', function (): void {
    global $wpdb;
    ...
    $archive_root = kiwi_create_temp_directory('kiwi_retention_archive_chunk_test');
    $service = new Kiwi_Retention_Sqlite_Archive_Service(new Kiwi_Test_Retention_Sqlite_Archive_Config($archive_root));
    $source = (new Kiwi_Retention_Source_Registry())->get('landing_page_sessions');
    ...
    try {
        ...
        kiwi_assert_same([201], $first['archived_primary_keys'] ?? [], 'Expected first chunk to archive the first primary key.');
        ...
        kiwi_assert_same(
            2,
            (int) $pdo->query('SELECT COUNT(*) FROM ' . $archive_table)->fetchColumn(),
            'Expected both chunked rows to remain in the source archive table.'
        );
        kiwi_assert_same(
            2,
...
kiwi_run_test('Kiwi_Retention_Coverage_Gate uses per-date light totals and passes exact coverage', function (): void {
    ...
    kiwi_assert_same('selective_deep_compare_v1', $result['coverage_mode'] ?? '', 'Expected gate to publish the selective deep-compare coverage mode.');
    kiwi_assert_same('2026-06-12 00:00:00', $result['effective_cutoff_value'] ?? '', 'Expected passed coverage to keep the requested cutoff.');
    kiwi_assert_same(['2026-06-11'], $result['deep_checked_dates'] ?? [], 'Expected the current retention edge date to be deep checked.');
    kiwi_assert_same([], $result['totals_only_dates'] ?? [], 'Expected no totals-only dates when the single candidate is the edge date.');
    kiwi_assert_same(['edge_date'], $result['deep_compare_reasons']['2026-06-11'] ?? [], 'Expected edge-date reason to be audited.');
...
kiwi_run_test('Kiwi_Retention_Coverage_Gate intentionally skips non-edge deep compares when light totals pass', function (): void {
    global $wpdb;

    $previous_wpdb = $wpdb ?? null;
    $wpdb = new Kiwi_Test_Wpdb_Retention_Coverage_Gate();
    $wpdb->candidate_dates = ['2026-06-09', '2026-06-10', '2026-06-11'];
    $wpdb->main_rows_by_date['2026-06-09'] = kiwi_test_retention_totals_row(['raw_sessions' => 3, 'summary_sessions' => 3]);
    $wpdb->main_rows_by_date['2026-06-10'] = kiwi_test_retention_totals_row(['raw_sessions' => 4, 'summary_sessions' => 4]);
    $wpdb->main_rows_by_date['2026-06-11'] = kiwi_test_retention_totals_row(['raw_sessions' => 5, 'summary_sessions' => 5]);
    $wpdb->tkzone_rows_by_date['2026-06-09'] = kiwi_test_retention_totals_row();
    ...
    kiwi_assert_same('passed', $result['status'], 'Expected non-edge deep mismatches not to be queried when light totals pass.');
    kiwi_assert_same(['2026-06-11'], $result['deep_checked_dates'] ?? [], 'Expected only the current retention edge date to be deep checked.');
    kiwi_assert_same(['2026-06-09', '2026-06-10'], $result['totals_only_dates'] ?? [], 'Expected older exact-total dates to remain totals-only by design.');
    kiwi_assert_same(['2026-06-09', '2026-06-10'], $result['deep_skipped_dates'] ?? [], 'Expected skipped deep dates to be auditable.');
    kiwi_assert_same(['edge_date'], $result['deep_compare_reasons']['2026-06-11'] ?? [], 'Expected edge deep-check reason to be auditable.');
    kiwi_assert_same([], $result['blocked_dates'] ?? [], 'Expected non-edge unqueried dimension mismatches not to block cleanup.');

    $wpdb = $previous_wpdb;
});

kiwi_run_test('Kiwi_Retention_Coverage_Gate limits CTA warning deep compares to two dates plus edge', function (): void {
    global $wpdb;

    $previous_wpdb = $wpdb ?? null;
    $wpdb = new Kiwi_Test_Wpdb_Retention_Coverage_Gate();
    $wpdb->candidate_dates = ['2026-06-09', '2026-06-10', '2026-06-11', '2026-06-12'];

    foreach ($wpdb->candidate_dates as $date) {
        $wpdb->main_rows_by_date[$date] = kiwi_test_retention_totals_row([
            'raw_cta1_click_events' => 1000,
            'summary_cta1_click_events' => 996,
        ]);
        $wpdb->tkzone_rows_by_date[$date] = kiwi_test_retention_totals_row();
        $wpdb->main_deep_warning_dates[] = $date;
    }

    $source = (new Kiwi_Retention_Source_Registry())->get('landing_page_sessions');

    $result = (new Kiwi_Retention_Coverage_Gate(new Kiwi_Config()))
        ->check_landing_page_sessions($source, '2026-06-13 00:00:00');

    kiwi_assert_same('passed', $result['status'], 'Expected tolerated CTA warning dates to pass retention coverage.');
    kiwi_assert_same(
        ['2026-06-12', '2026-06-09', '2026-06-10'],
        $result['deep_checked_dates'] ?? [],
        'Expected the edge date plus the first two CTA warning dates to be deep checked.'
    );
    kiwi_assert_same(['2026-06-11'], $result['totals_only_dates'] ?? [], 'Expected later CTA warnings beyond the budget to be totals-only.');
    kiwi_assert_same(['edge_date'], $result['deep_compare_reasons']['2026-06-12'] ?? [], 'Expected edge reason for the newest date.');
    kiwi_assert_same(['cta_warning'], $result['deep_compare_reasons']['2026-06-09'] ?? [], 'Expected first CTA warning reason.');
    kiwi_assert_same(['cta_warning'], $result['deep_compare_reasons']['2026-06-10'] ?? [], 'Expected second CTA warning reason.');
...
kiwi_run_test('Kiwi_Retention_Coverage_Gate requires current TK-zone PID-set coverage', function (): void {
    ...
    kiwi_assert_contains('FROM landing_loads l', $queries, 'Expected TK-zone handoff coverage to restrict handoff aggregation to candidate landing sessions.');
    kiwi_assert_contains('INNER JOIN wp_kiwi_landing_handoff_events h', $queries, 'Expected TK-zone handoff coverage not to materialize unrelated handoff sessions.');
...
kiwi_run_test('Kiwi_Retention_Cleanup_Service dry-runs handoff retention without a coverage-gate call', function (): void {
    global $wpdb;
    ...
    $gate = new Kiwi_Test_Retention_Coverage_Gate(['status' => 'failed']);
    $service = new Kiwi_Test_Retention_Cleanup_Service(
        new Kiwi_Config(),
        new Kiwi_Retention_Source_Registry(),
        $runs,
    ...
    $service->eligible_rows = 7;

    $result = $service->run_source('landing_handoff_events', 'manual');
    $run_row = $runs->rows[1] ?? [];

    kiwi_assert_same(true, $result['success'] ?? false, 'Expected handoff dry-run to succeed.');
    kiwi_assert_same('success', $result['status'] ?? '', 'Expected handoff dry-run to finish without worker work.');
    kiwi_assert_same('not_required', $result['gate_status'] ?? '', 'Expected an explicit no-coverage policy result.');
    kiwi_assert_same([], $gate->calls, 'Expected zero landing-session coverage-gate calls for handoff retention.');
    kiwi_assert_same('2026-03-25 00:00:00', $result['cutoff_value'] ?? '', 'Expected the seven-day minimum to preserve complete calendar days.');
    kiwi_assert_same(7, $run_row['retention_days_effective'] ?? 0, 'Expected explicit handoff retention below seven days to clamp to seven.');
    kiwi_assert_same('wp_kiwi_landing_handoff_events', $run_row['source_table'] ?? '', 'Expected the handoff audit to retain its strict source table.');
    kiwi_assert_same([], $archive->calls, 'Expected handoff dry-run not to use the SQLite archive.');
    kiwi_assert_same([], $archive->chunk_calls, 'Expected handoff dry-run not to archive a worker chunk.');
    kiwi_assert_same([], $service->deleted_primary_keys, 'Expected handoff dry-run not to delete MySQL rows.');
    kiwi_assert_same(['before_cleanup', 'after_cleanup'], array_column($snapshots->snapshots, 'snapshot_phase'), 'Expected handoff dry-run to preserve scope snapshots.');

    $wpdb = $previous_wpdb;
});

kiwi_run_test('Kiwi_Retention_Cleanup_Service resumes receipt-backed handoff chunks without touching sessions', function (): void {
    global $wpdb;
    ...
    $service->target_max_primary_key = 13;
    $service->delete_result = ['deleted_rows' => 2, 'delete_batches' => 1];

    try {
        $scheduled = $service->run_source('landing_handoff_events', 'cron');
        $first_worker = $service->run_worker('landing_handoff_events');
        $second_worker = $service->run_worker('landing_handoff_events');

        kiwi_assert_same('pending', $scheduled['status'] ?? '', 'Expected handoff scheduler to create bounded worker state.');
        kiwi_assert_same('partial', $first_worker['status'] ?? '', 'Expected first handoff chunk to remain resumable.');
        kiwi_assert_same('completed', $second_worker['status'] ?? '', 'Expected second handoff chunk to complete.');
        kiwi_assert_same([], $gate->calls, 'Expected no coverage-gate call across handoff scheduler and workers.');
        kiwi_assert_same([10, 11, 12, 13], $service->deleted_primary_keys, 'Expected only receipt-backed handoff IDs to be deleted across resumes.');
        kiwi_assert_same(
            ['wp_kiwi_landing_handoff_events', 'wp_kiwi_landing_handoff_events'],
            array_map(static function (array $call): string {
                return (string) ($call['source']['source_table'] ?? '');
            }, $archive->chunk_calls),
            'Expected every archived chunk to stay inside the handoff table.'
        );
        kiwi_assert_same(11, $archive->chunk_calls[1]['last_primary_key'] ?? 0, 'Expected the second handoff worker to resume at the persisted archive cursor.');
        kiwi_assert_same(
            sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'kiwi_retention_archive_2026.sqlite',
            $archive->chunk_calls[1]['archive_db_path'] ?? '',
            'Expected handoff resume to reuse the frozen archive generation.'
        );
        kiwi_assert_same(13, $runs->rows[1]['archive_last_primary_key'] ?? 0, 'Expected final handoff archive cursor persistence.');
        kiwi_assert_same(13, $runs->rows[1]['delete_last_primary_key'] ?? 0, 'Expected final handoff delete cursor persistence.');
    } finally {
        @unlink(sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'kiwi_retention_archive_2026.sqlite.lock');
        $wpdb = $previous_wpdb;
    }
});

kiwi_run_test('Kiwi_Retention_Cleanup_Service isolates the handoff worker lock by source key', function (): void {
    global $wpdb;
    ...
    $GLOBALS['kiwi_test_options'] = [
        'kiwi_retention_settings' => [
            'landing_handoff_events' => ['enabled' => true, 'dry_run' => false, 'retention_days' => 21],
        ],
    ];
    $runs = new Kiwi_Test_Retention_Cleanup_Run_Repository();
    $archive = new Kiwi_Test_Retention_Sqlite_Archive_Service();
    $service = new Kiwi_Test_Retention_Cleanup_Service(
        ...
        new Kiwi_Test_Retention_Table_Growth_Snapshot_Repository(),
        $archive,
        new Kiwi_Test_Retention_Coverage_Gate(['status' => 'failed'])
    );
    $service->eligible_rows = 2;
    $service->target_max_primary_key = 2;
    $service->run_source('landing_handoff_events', 'cron');
    $GLOBALS['kiwi_test_transients']['kiwi_retention_cleanup_worker_lock_landing_handoff_events'] = '1';

    $result = $service->run_worker('landing_handoff_events');

    kiwi_assert_same('lock_skipped', $result['worker_phase'] ?? '', 'Expected the handoff-specific worker lock to defer only that source.');
    kiwi_assert_same([], $archive->chunk_calls, 'Expected the locked handoff worker not to archive.');
    kiwi_assert_same('1', $GLOBALS['kiwi_test_transients']['kiwi_retention_cleanup_worker_lock_landing_handoff_events'] ?? '', 'Expected the handoff lock to remain owned by its current worker.');
    kiwi_assert_same(false, isset($GLOBALS['kiwi_test_transients']['kiwi_retention_cleanup_worker_lock_landing_page_sessions']), 'Expected no landing-session worker lock leakage.');

    $wpdb = $previous_wpdb;
});

kiwi_run_test('Kiwi_Retention_Cleanup_Service blocks a handoff delete when corruption appears after receipt commit', function (): void {
    global $wpdb;
    ...
    $archive->chunks[] = [
        ...
        'archive_inserted_rows' => 2,
        'archive_duplicate_rows' => 0,
        'archived_primary_keys' => [21, 22],
        'last_primary_key' => 22,
        'has_more' => false,
    ];
    $corruption_gate = new Kiwi_Test_Sequenced_Retention_Corruption_Safety_Gate_Coordinator();
    $corruption_gate->block_on_inspection = 3;
    $service = new Kiwi_Test_Retention_Cleanup_Service(
        new Kiwi_Config(),
        new Kiwi_Retention_Source_Registry(),
        $runs,
    ...
    $service->eligible_rows = 2;
    $service->target_max_primary_key = 22;
    $service->delete_result = ['deleted_rows' => 2, 'delete_batches' => 1];
    ...
    try {
        ...
        kiwi_assert_same(3, $corruption_gate->inspection_count, 'Expected the final corruption inspection immediately before the handoff delete.');
        kiwi_assert_same(1, count($archive->chunk_calls), 'Expected the handoff receipt to commit before the synthetic corruption block.');
        kiwi_assert_same([], $service->deleted_primary_keys, 'Expected corruption after receipt commit to block every handoff MySQL delete.');
        kiwi_assert_same('archive_corruption_write_blocked', $result['error_code'] ?? '', 'Expected the shared corruption reason code.');
        kiwi_assert_same('archive_corruption_blocked', $runs->rows[1]['worker_phase'] ?? '', 'Expected the handoff run to remain safely resumable behind the corruption gate.');
    } finally {
        @unlink(sys_get_temp_dir() . DIRECTORY_SEPARATOR . 'kiwi_retention_archive_2026.sqlite.lock');
        $wpdb = $previous_wpdb;
    }
...
kiwi_run_test('Kiwi_Retention_Cleanup_Service uses effective cutoff when scheduling partial coverage cleanup', function (): void {
    global $wpdb;
    ...
    $service->target_max_primary_key = 404;

    $result = $service->run_source('landing_page_sessions', 'wp_cli');
    $run_row = $runs->rows[1] ?? [];

    kiwi_assert_same('pending', $result['status'], 'Expected partial coverage to schedule worker cleanup up to the effective cutoff.');
    kiwi_assert_same('partial', $result['gate_status'], 'Expected partial gate status to be audited.');
    kiwi_assert_same(['count', 'count', 'target'], $events, 'Expected scheduler to recount effective rows and freeze the target primary key without archiving.');
    kiwi_assert_same([], $archive->calls, 'Expected scheduler not to run full archive work.');
    kiwi_assert_same([], $archive->chunk_calls, 'Expected scheduler not to run chunk archive work.');
    ...
    kiwi_assert_same(404, $run_row['target_max_primary_key'] ?? 0, 'Expected scheduler to freeze the target primary key for the worker run.');
    kiwi_assert_same('archive_pending', $run_row['worker_phase'] ?? '', 'Expected scheduler to mark the worker phase as archive pending.');
    kiwi_assert_true(!empty($result['schedule_worker']), 'Expected scheduler result to request worker scheduling.');
    ...
    kiwi_assert_same(['before_cleanup'], array_column($snapshots->snapshots, 'snapshot_phase'), 'Expected scheduler to defer the after snapshot until worker completion.');

    $wpdb = $previous_wpdb;
});

kiwi_run_test('Kiwi_Retention_Cleanup_Service fails closed when target primary key cannot be frozen', function (): void {
    global $wpdb;
    ...
    $service->events =& $events;
    $service->eligible_rows = 3;
    $service->target_max_primary_key = 0;

    $result = $service->run_source('landing_page_sessions', 'cron');
    $run_row = $runs->rows[1] ?? [];

    kiwi_assert_same(false, $result['success'], 'Expected missing target primary key to fail closed for eligible rows.');
    kiwi_assert_same('failed', $result['status'], 'Expected missing target primary key to persist a failed run.');
    kiwi_assert_same('target_primary_key_unavailable', $result['error_code'], 'Expected target primary-key failure to be explicit.');
    kiwi_assert_same('target_key_unavailable', $run_row['worker_phase'] ?? '', 'Expected failed target freeze phase to be audited.');
    kiwi_assert_true(empty($result['schedule_worker']), 'Expected missing target primary key not to schedule worker work.');
    kiwi_assert_same([], $archive->calls, 'Expected missing target primary key not to archive rows.');
    kiwi_assert_same([], $archive->chunk_calls, 'Expected missing target primary key not to archive chunks.');
    kiwi_assert_same(['count', 'target'], $events, 'Expected scheduler to stop immediately after target freeze failure.');
    kiwi_assert_same(['before_cleanup', 'after_cleanup'], array_column($snapshots->snapshots, 'snapshot_phase'), 'Expected failed target freeze to leave before/after audit snapshots.');

    $wpdb = $previous_wpdb;
});

kiwi_run_test('Kiwi_Retention_Cleanup_Service worker archives chunks before deleting landing-page-session rows', function (): void {
    global $wpdb;
    ...
    kiwi_assert_same('completed', $result['status'], 'Expected worker cleanup to complete when archive and gates pass.');
    kiwi_assert_same(['count', 'target', 'archive_chunk', 'delete'], $events, 'Expected cleanup to archive the chunk before deleting it.');
...
kiwi_run_test('Kiwi_Retention_Cleanup_Service worker resumes after partial archive progress', function (): void {
    global $wpdb;
    ...
    $scheduled = $service->run_source('landing_page_sessions', 'cron');
    $first_worker = $service->run_worker('landing_page_sessions');
    $second_worker = $service->run_worker('landing_page_sessions');

    kiwi_assert_same('pending', $scheduled['status'], 'Expected scheduler to create a pending run.');
    kiwi_assert_same('partial', $first_worker['status'], 'Expected first worker chunk to leave partial progress.');
...
kiwi_run_test('Kiwi_Retention_Cleanup_Service worker lock skip reschedules without work', function (): void {
    global $wpdb;
    ...
    $service->run_source('landing_page_sessions', 'cron');
    $GLOBALS['kiwi_test_transients']['kiwi_retention_cleanup_worker_lock_landing_page_sessions'] = '1';
    $result = $service->run_worker('landing_page_sessions');

    kiwi_assert_same('lock_skipped', $result['worker_phase'], 'Expected active worker lock to produce a lock skip.');
    kiwi_assert_true(!empty($result['schedule_worker']), 'Expected lock skip to request worker rescheduling.');
...
kiwi_run_test('Kiwi_Retention_Cleanup_Service fails closed when worker cannot persist running state', function (): void {
    ...
    $service->events =& $events;
    $service->eligible_rows = 3;
    $service->target_max_primary_key = 303;
    ...
    kiwi_assert_same('pending', $scheduled['status'], 'Expected scheduler to create the worker run before the audit failure simulation.');
    kiwi_assert_same(false, $result['success'], 'Expected worker result to fail when running state cannot be persisted.');
    kiwi_assert_same('failed', $result['status'], 'Expected audit update failure to mark the returned worker result failed.');
    kiwi_assert_same('run_audit_update_failed', $result['error_code'], 'Expected running-state persistence failures to be explicit.');
    kiwi_assert_same(false, $result['audit_persisted'] ?? true, 'Expected returned result to expose the missing audit persistence.');
    kiwi_assert_same('failed', $result['cleanup_status_before_audit_failure'] ?? '', 'Expected returned result to retain the attempted worker failure status.');
...
kiwi_run_test('Kiwi_Retention_Cleanup_Service preserves receipt audit for delete retry on count mismatch', function (): void {
    ...
    $service->eligible_rows = 2;
    $service->target_max_primary_key = 202;
    $service->delete_result = ['deleted_rows' => 1, 'delete_batches' => 1];

    $scheduled = $service->run_source('landing_page_sessions', 'manual');
    $result = $service->run_worker('landing_page_sessions');
...
kiwi_run_test('Kiwi_Retention_Cleanup_Service does not stale-fail a delayed pending worker run', function (): void {
    global $wpdb;
