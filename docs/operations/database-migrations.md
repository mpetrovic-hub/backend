...
During `apply`, the repository verifies that the new unique index exists with the complete ordered identity before removing every unique index that enforces the narrower four-column legacy identity, including renamed or reordered copies. Existing rows therefore remain grouped as `legacy`, and an old constraint is not removed unless its replacement is already valid. Post-apply `status` also rejects that legacy identity by its order-independent column set rather than relying only on its historical `variant_summary` name.

Both `status` and the `apply` preflight also require every existing `allocation_version` value to be non-null and non-blank. A partial predecessor schema with null, empty, or whitespace-only values returns `legacy_migration_required` before mutation. The generic runner deliberately does not guess or backfill those rows; recovery requires a reviewed migration-specific artifact that establishes the intended historical identity.
