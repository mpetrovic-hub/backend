...
`status` reads controller state, archive discovery, the resolved active generation (or its lookup error), and sanitized relevant open Incidents without mutation. It validates an open run's frozen archive path without creating the configured SQLite archive directory when that root is absent. Relevant Incident types are filtered in the repository before their bounded per-type limits are applied, so unrelated operational volume cannot hide retention health, corruption, or receipt Incidents; a failed Incident query makes `status` fail closed instead of returning an empty list. `diagnose` accepts an exact discovered archive basename and `--check=quick|integrity`; it does not accept arbitrary paths. `preflight` verifies PDO SQLite, process supervision, the shared non-blocking lock, atomic state exchange, a scratch SQLite check, and child cleanup. Its production-shaped scratch database lives in a hidden unique subdirectory, outside the root-only archive discovery surface, and is removed after the check.
...
The controller interprets calendar dates and weekdays in `Europe/Berlin`: Monday through Saturday use `quick_check`; Sunday uses `integrity_check`. The daily target is the frozen `archive_db_path` of the oldest open cleanup run when one exists, even across a calendar-year rollover; otherwise it is the latest current-year generation. A failed archive-directory scan, active-run lookup, invalid frozen path, or missing frozen archive after persisted archive/delete progress fails closed instead of checking a guessed fallback or recording `no_work`. Once any incomplete daily attempt has persisted an exact archive target, that target disappearing also fails closed on the same Berlin date and remains audited for the bounded retries. Target-resolution failures, WordPress loader exceptions, exceptions from the scheduled service call, and other scheduled runner failures count toward the same three daily attempts and raise the same central incomplete-check incident on the third slot. If a prior controller still owns the non-blocking lock, every overlapping scheduled or bootstrap-failure invocation atomically persists a fixed deferral receipt and exits nonzero; the current or next lock owner reconciles each receipt exactly once into the strict state before deleting it, including the crash window after state persistence. Both the complete health service and its dependency-independent fallback reconcile older receipts before aligning or advancing the current bootstrap-failure state, including across Berlin calendar days. If the normal Operational Event service is the missing class, the fallback persists the fixed, sanitized Incident directly through WordPress database access. A missing path before any daily target was selected remains `no_work`; that result does not resolve an earlier lookup Incident because no SQLite check completed. The second and third slots retry incomplete/deferred work. After the third incomplete attempt, it raises `retention_archive_health_check_incomplete`; a later daily attempt cycle records and reports `repeated` while that Incident remains open. The next Berlin calendar day resets only that retry budget and checks the persisted overdue archive and mode before initializing the new day's target. If a direct-daily corruption marker is recovered on a later Berlin date after its controller-state write failed, the reconstructed result keeps the marker's original Berlin detection date, so the next slot still runs the current day's active-generation check. A later complete daily or annual result for that archive resolves the incident, including an annual retry that reconciles and immediately acknowledges an already-persisted quarantine marker after durable state, and the one-line command JSON reports the actual `raised`, `repeated`, or `resolved` append-only transition. Annual entries remain pending and retryable until any required incomplete-Incident recovery is durably persisted; a failed correlation lookup is distinct from a confirmed absence of an open Incident and therefore remains retryable too. An annual marker retry that crosses into a new Berlin day waits until that day's active-archive check is durably complete and never replaces its daily state. An already-acknowledged quarantine marker reports no Incident action unless it actually resolves an incomplete Incident. From January 2 onward, free slots after the daily check process one file from the annual integrity campaign snapshot. The strict annual audit state stores the cycle year, campaign start and completion timestamps, stable sorted relative archive names, and each file's status, `integrity` check mode, and last definitive result. Completed files are not run again; deferred, inconclusive, unavailable, or otherwise nonfinal files remain open, and the campaign completes only when every frozen entry has a final result. If a file in that frozen snapshot later becomes unavailable, it remains pending and the controller fails with `annual_archive_unavailable`; the file is never recorded as skipped. An unfinished campaign remains the active frozen snapshot across January 1 and must complete before the controller initializes the new calendar-year campaign.
...
A completed corruption result first raises `retention_archive_corruption_detected`, then writes a `.quarantine.json` marker beside the affected generation. The marker receives `controller_recorded_at` only after the controller has durably recorded the corruption result in its strict state and any required incomplete-check recovery; direct daily corruption requires both lookup-scoped and archive-scoped recovery persistence before acknowledgement. A marker-write failure or recovery-persistence failure remains visible and is retried on restart. On restart, an unacknowledged predecessor marker is reconciled before a newly active successor can be selected. Incomplete or timed-out checks never quarantine. The worker never falls back to an older archive. If its persisted generation is quarantined, it derives the deterministic `_part_N` successor from that exact archive's year and generation, even after a calendar-year rollover, then atomically closes the old cleanup run and creates the successor for remaining MySQL rows using the existing cleanup-run schema. Transient failures while resolving the successor path, counting remaining MySQL rows, or persisting the atomic successor transition keep the original run open and reschedule the transition. The append-only transition event is mandatory and is retried idempotently before the successor may delete source rows. The first non-empty verified receipt/delete/audit batch in the replacement generation resolves the corruption incident with `resolution_reason=quarantined_and_replaced`. If the deterministic successor has zero remaining rows, its completed cleanup row retains the unresolved recovery context by source until a later ordinary cleanup run produces that qualifying batch, including after a calendar-year rollover; successful resolution marks the carried context durably so later batches do not scan it again. If that replacement is itself quarantined before its first non-empty batch, the next qualifying generation walks the quarantined predecessor chain and resolves every still-open carried corruption correlation. If no cleanup run was open when the active archive was quarantined, the first later non-empty verified batch derives the same correlation from every acknowledged quarantine marker, including markers from an earlier archive year. Existing rows already deleted into the corrupt archive cannot be reconstructed automatically and remain an operator-review concern.
