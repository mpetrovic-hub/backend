...
`check` selects the frozen archive of the oldest open cleanup run, otherwise the newest discovered archive, and performs the explicitly requested mode. It is the only mode intended for automatic scheduling. `diagnose` accepts an exact discovered archive basename and is strictly read-only: path-like input is rejected rather than reduced to its basename, and the command neither raises or resolves Incidents nor changes the write-block sentinel. `unblock` applies the same strict basename rule, always requires `--confirm`, always completes an explicit read-only `integrity_check`, and only then changes safety state. Without `--replacement`, it verifies and re-enables the same archive. With `--replacement=B`, `B` must be a later generation from the same year; the command verifies `B`, terminalizes unfinished cleanup runs still bound to `A`, clears `A`'s sentinel, and resolves the corruption Incident last. No mode creates a replacement archive.
