...
Every command prints exactly one compact JSON line. Mandatory fields are `schema_version`, `command`, `result`, `reason_code`, `archive`, `check`, `started_at`, `finished_at`, and `duration_seconds`. Optional fields are `incident_action`, `write_blocked`, and `child_running`. Filesystem paths, exception text, database credentials, and a duplicate JSON `exit_code` are never emitted. The child opens SQLite with an immutable read-only URI under the generation lock, so checks do not create `-wal` or `-shm` sidecars. A pre-existing non-empty WAL is not ignored: it fails closed with `sqlite_wal_not_empty` and exit `2` before PRAGMA execution. Process exit mapping:
