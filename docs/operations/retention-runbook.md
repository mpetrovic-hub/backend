...
`check` selects the frozen archive of the oldest open cleanup run, otherwise the newest discovered archive, and performs the explicitly requested mode. It is the only mode intended for automatic scheduling. `diagnose` accepts an exact discovered archive basename and is strictly read-only: path-like input is rejected rather than reduced to its basename, and the command neither raises or resolves Incidents nor changes the write-block sentinel. `unblock` applies the same strict basename rule, always requires `--confirm`, always completes an explicit read-only `integrity_check`, and only then changes safety state. Without `--replacement`, it verifies and re-enables the same archive. With `--replacement=B`, `B` must be a later generation from the same year and must be the generation the cleanup resolver would select. The command temporarily write-blocks `B`, terminalizes unfinished cleanup runs still bound to `A`, clears `A`'s sentinel, and resolves `A`'s corruption Incident as its last recovery action. `B` remains blocked if that resolution fails; its temporary transition block is removed only after the resolution succeeds. No mode creates a replacement archive.
