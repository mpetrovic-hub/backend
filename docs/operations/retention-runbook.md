...
`check` selects the frozen archive of the oldest open cleanup run, otherwise the newest discovered archive, and performs the explicitly requested mode. It is the only mode intended for automatic scheduling. `diagnose` accepts an exact discovered archive basename and is strictly read-only: path-like input is rejected rather than reduced to its basename, and the command neither raises or resolves Incidents nor changes the write-block sentinel. `unblock` applies the same strict basename rule, always requires `--confirm`, always completes an explicit read-only `integrity_check`, and only then changes safety state. A matching corruption sentinel or open Corruption Incident must already exist on `A`; a healthy archive cannot enter recovery or terminalize resumable runs. Without `--replacement`, it verifies and re-enables the same archive. With `--replacement=B`, `B` must be a later generation from the same year and must be the generation the cleanup resolver would select. The command writes a distinct replacement-transition marker on `B` bound to source `A`, terminalizes unfinished cleanup runs still bound to `A`, clears `A`'s corruption sentinel, and resolves `A`'s Corruption Incident as its last recovery action. `B` remains blocked if that resolution fails; its transition marker is removed only after the resolution succeeds and is never treated by Health as corruption evidence for `B`. The source binding permits only the matching final-clear retry if `A`'s own gate is already resolved. No mode creates a replacement archive.
...
- replacement archive `B`: build and validate `B` outside the runtime, then run `unblock --archive=A --replacement=B --confirm`; the command accepts only the later same-year generation the cleanup resolver would select, verifies `B` before changing run state, and keeps `B` durably blocked until `A`'s final Incident resolution succeeds.
