...
`check` selects the frozen archive of the oldest open cleanup run, otherwise the newest discovered archive, and performs the explicitly requested mode. It is the only mode intended for automatic scheduling. `diagnose` accepts an exact discovered archive basename and is strictly read-only: it neither raises or resolves Incidents nor changes the write-block sentinel. `unblock` always requires `--confirm`, always completes an explicit read-only `integrity_check`, and only then changes safety state. Without `--replacement`, it verifies and re-enables the same archive. With `--replacement=B`, `B` must be a later generation from the same year; the command verifies `B`, terminalizes unfinished cleanup runs still bound to `A`, clears `A`'s sentinel, and resolves the corruption Incident last. No mode creates a replacement archive.
...
Any non-definitive scheduled `check` raises or repeats one sanitized `retention_archive_health_unavailable` Operational Incident. The next definitive check resolves that availability correlation. If bootstrap reaches the normal controller and Operational Event service, it records the same Availability correlation. The dependency-independent fallback emits only sanitized JSON and never writes controller state, SQLite receipts, an Incident spool, or a direct database fallback. The operator decides when to retry from the compact exit code and `reason_code`.
...
- replacement archive `B`: build and validate `B` outside the runtime, then run `unblock --archive=A --replacement=B --confirm`; the command accepts only a later generation from the same archive year and verifies `B` before changing run state.
