...
Any non-definitive scheduled `check` raises or repeats one sanitized `retention_archive_health_unavailable` Operational Incident unless the latest persisted recovery belongs to a later-started definitive check. The comparison runs under the short Availability lifecycle lock and uses only internal operation ordering in existing event context; it creates no check state, receipt, table, or routine success event. Without a later persisted recovery, a timeout still opens Availability even when its child has ended and the generation lock is free. `archive_lock_active` retains its additional freshness guard: immediately before that transition, the controller probes the generation lock once without waiting. If the lock is still active, the Incident transition remains eligible; if it has become available, the controller briefly acquires and releases it and suppresses only the stale lock transition. The original command remains one-shot and never retries, waits, or opens SQLite again. The next definitive check resolves an actually open availability correlation. If a corruption sentinel or open Corruption Incident is already durable but that resolution write failed, the next controller call retries the resolution before returning the existing blocked result and does not rerun SQLite. A replacement-transition marker by itself is not definitive corruption evidence and never resolves Availability. If bootstrap reaches the normal controller and Operational Event service, it records the same Availability correlation. The dependency-independent fallback emits only sanitized JSON and never writes controller state, SQLite receipts, an Incident spool, or a direct database fallback. The operator decides when to retry from the compact exit code and `reason_code`.
