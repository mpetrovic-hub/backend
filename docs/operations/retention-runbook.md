...
The parent process starts one dedicated read-only SQLite child and enforces `KIWI_RETENTION_ARCHIVE_HEALTH_TIMEOUT_SECONDS` (default `600` seconds; accepted range `30..3600`). The child acquires the same exclusive, non-waiting generation lock used by retention cleanup before it opens SQLite and holds the lock until it exits. After lock acquisition it rechecks the corruption sentinel and replacement-transition marker before opening SQLite, closing the controller-check-to-child-lock window. Scheduled `check` executions defer behind either valid gate; only an explicitly confirmed `unblock` verification may run its required read-only `integrity_check` behind an existing well-formed gate. A cleanup lock therefore returns `archive_lock_active`, and a surviving timed-out child continues to prevent SQLite writes and MySQL deletion after the parent returns. The parent never terminates a started child at the timeout boundary, including before readiness has been observed; it returns the inconclusive timeout with `child_running=true` and lets the child complete its lock/gate protocol. A timeout or exception is inconclusive and never proof of corruption. After a healthy child result, the controller rechecks the central gate before resolving Availability, so an Incident-only or transition gate established during the child window still blocks success. Only a completed PRAGMA whose result is not exactly `ok` can set the generation-specific write-block sentinel and raise `retention_archive_corruption_detected`. The sentinel is written before the Incident. If that filesystem write fails, the child retains the lock and requests a parent handoff; the parent persists the Corruption Incident and acknowledges it before the child releases. If both durable gates are temporarily unavailable, the parent reports a technical error while the child retains the OS lock and retries the sentinel until one durable corruption gate exists. Each recovered-then-reopened corruption uses a fresh lifecycle-scoped idempotency key. Reconciliation between a lone sentinel and Incident acquires the generation lock and rechecks both sources before writing, so it cannot reopen a concurrently completed recovery. Before persisting a gate from a completed PRAGMA, the parent captures either existing corruption source, including the child's reported sentinel, and rechecks both under the generation lock. If both are now clear, only a later persisted `resolved` with `manual_repair_verified` or `manual_replacement_verified` proves that `unblock` superseded the old check; otherwise the missing gate is restored fail-closed. A sentinel-only `unblock` first reconciles its Corruption Incident and stops without clearing the sentinel if that audit cannot persist. Failure of both gates returns an error and never success. Cleanup fails closed if either safety source is unreadable or disagrees; unreadable or malformed gate state, including a malformed replacement-transition marker, is a technical exit `2` and raises Availability rather than masquerading as an expected corruption block.
...
Any non-definitive scheduled `check` raises or repeats one sanitized `retention_archive_health_unavailable` Operational Incident unless the latest persisted recovery belongs to a later-started definitive check. A definitive result likewise cannot resolve an open failure from a later-started check. Both comparisons run under the short Availability lifecycle lock and use only internal operation ordering in existing event context; they create no check state, receipt, table, or routine success event. Without a later persisted recovery, a timeout still opens Availability even when its child has ended and the generation lock is free. `archive_lock_active` retains its additional freshness guard: immediately before that transition, the controller probes the generation lock once without waiting. If the lock is still active, the Incident transition remains eligible; if it has become available, the controller briefly acquires and releases it and suppresses only the stale lock transition. The original command remains one-shot and never retries, waits, or opens SQLite again. The next definitive check resolves an actually open availability correlation only when that failure is not from a later-started check. If a corruption sentinel or open Corruption Incident is already durable but that resolution write failed, the next controller call retries the resolution before returning the existing blocked result and does not rerun SQLite. A replacement-transition marker by itself is not definitive corruption evidence and never resolves Availability. If bootstrap reaches the normal controller and Operational Event service, it records the same Availability correlation. The dependency-independent fallback emits only sanitized JSON and never writes controller state, SQLite receipts, an Incident spool, or a direct database fallback. The operator decides when to retry from the compact exit code and `reason_code`.
