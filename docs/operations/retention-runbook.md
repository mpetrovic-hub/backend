...
Any non-definitive scheduled `check` raises or repeats one sanitized `retention_archive_health_unavailable` Operational Incident. The next definitive check resolves that availability correlation. If a corruption sentinel or open Corruption Incident is already durable but that resolution write failed, the next controller call retries the resolution before returning the existing blocked result and does not rerun SQLite. A replacement-transition marker by itself is not definitive corruption evidence and never resolves Availability. If bootstrap reaches the normal controller and Operational Event service, it records the same Availability correlation. The dependency-independent fallback emits only sanitized JSON and never writes controller state, SQLite receipts, an Incident spool, or a direct database fallback. The operator decides when to retry from the compact exit code and `reason_code`.
