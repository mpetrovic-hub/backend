...
Any non-definitive scheduled `check` raises or repeats one sanitized `retention_archive_health_unavailable` Operational Incident, subject to one freshness guard for `archive_lock_active`. Immediately before that lifecycle transition, the controller holds the short Availability lifecycle lock and probes the generation lock once without waiting. If the generation lock is still active, the Incident transition is persisted before a concurrent definitive recovery can follow. If the generation lock has become available, the controller briefly acquires and releases it and suppresses the now-stale Incident transition. The original command still returns one-shot `deferred` with `archive_lock_active`; it does not retry, wait, or open SQLite again. The next definitive check resolves an actually open availability correlation. If a corruption sentinel or open Corruption Incident is already durable but that resolution write failed, the next controller call retries the resolution before returning the existing blocked result and does not rerun SQLite. A replacement-transition marker by itself is not definitive corruption evidence and never resolves Availability. If bootstrap reaches the normal controller and Operational Event service, it records the same Availability correlation. The dependency-independent fallback emits only sanitized JSON and never writes controller state, SQLite receipts, an Incident spool, or a direct database fallback. The operator decides when to retry from the compact exit code and `reason_code`.
