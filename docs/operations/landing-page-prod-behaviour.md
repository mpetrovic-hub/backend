...
- `wp_kiwi_landing_funnel_daily_summary`
  ...
  - derives session service/provider/flow/country/source/device dimensions and IP buckets from deduplicated `wp_kiwi_landing_page_sessions`; engagement and handoff rows contribute metrics but do not create fallback summary sessions, repair missing session dimensions, or parse user-agent fields in SQL
