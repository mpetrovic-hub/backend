...
The closed-to-`raised` decision and recovery read/write are serialized by one short, bounded MySQL advisory lock derived from a hash of the `correlation_key`. A producer may attach a current-state predicate to a failure transition; that predicate and its accepted `raised` or `repeated` write run while the lifecycle lock is held, so evidence that became stale before persistence can be suppressed. Without such a predicate, an already-open correlation releases the lock before the existing conditional `repeated` append. This preserves append-only lifecycle ordering across overlapping producers without adding a mutable incident row, a Health state file, or routine `resolved` rows while no incident is open. Lock acquisition, release, predicate, or accepted-write failure is a persistence failure; it is never treated as a successful lifecycle change.
