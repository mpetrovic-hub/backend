...
- normalized device dimensions (`device_brand`, `os`, `os_version`, `browser`)
...
- landing-session client-IP context (`client_ip`, stored `client_ip_version`, stored `client_ip_prefix`, optional `client_ip_hash`)
...
The shared statistics report is exposed through the protected `[kiwi_statistics]` shortcode. Its primary UI and CSV export path reads from the persistent `wp_kiwi_landing_funnel_daily_summary` table so larger date ranges can be filtered and pivoted without rebuilding the old transition views on every request. The report supports normal date-range filters plus `service_key`, `landing_key`, `tksource`, `device_brand`, `os`, `os_version`, and `browser` dropdown filters. `tkzone` is intentionally excluded from the main summary UI/CSV path and remains available through the separate tkzone daily summary and legacy/debug views. Coarse IP dimensions remain output columns and can still be used by internal diagnostic URL filters, but they are not offered as normal dropdown filters because `client_ip_prefix` is cardinality-sensitive.

The shortcode and CSV export share one internal statistics-read contract and one column list. The current columns are the summary dimensions (`metric_date`, landing/service/provider/flow/country/source/device buckets, `client_ip_version`, and coarse `client_ip_prefix`) plus daily metrics for sessions, page loads, CTA1/CTA2/CTA3 sessions and click events, handoff attempts/successes/fails/rate, hidden-time min/max, sales, and `sales_amount_minor`. The daily summary does not store sale ID, transaction ID drilldown lists, `tkzone`, hidden-time median, raw `client_ip`, or `client_ip_hash`; those legacy, high-cardinality, or personal-data-heavy columns are intentionally not emitted by the main summary read path.
...
The legacy `wp_kiwi_v_one_for_all` view still exposes `device_brand`, `android_version`, and `browser` computed from raw UA fields for session rows. The primary daily summary and new sale snapshots use the shared normalizer instead, persisting `device_brand`, `os`, `os_version`, and `browser`. `device_brand` is populated from exact DB-backed model mappings first, then known manufacturer rules such as Samsung, Huawei, Honor, Google Pixel, Xiaomi-family models, and Apple mobile devices; unknown model strings are grouped as `(unknown)` instead of being promoted into new brands. The view exposes `landing_key`, `service_key`, `tksource`, `tkzone`, those legacy device/browser dimensions, session/load/CTA counters, handoff attempts/successes/fails/rate, hidden-time aggregates, and completed sales.

`wp_kiwi_landing_page_sessions` is the canonical source for server-known session dimensions. The landing-page router writes `provider_key`, `flow_key`, campaign/service `country`, `pid`, `tksource`, `tkzone`, normalized primary `browser_language`, normalized device buckets (`device_brand`, `os`, `os_version`, `browser`), and coarse client-IP buckets (`client_ip_version`, `client_ip_prefix`) on the first server request. Source parameters are stored even when no `click_id` is present. Client IPs are resolved by `Kiwi_Client_Ip_Resolver`: forwarded headers are accepted only when the direct peer matches explicit trusted proxy configuration, and missing or invalid values use `(unknown)` buckets. If the direct peer is trusted but no usable forwarded client candidate is present, the resolver keeps the client-IP bucket `(unknown)` instead of treating the proxy address as the client. `browser_language` stores only the primary language tag such as `fr`, `de`, `en`, or `ar`; empty or invalid headers use `(unknown)`. When `page_loaded` UA Client Hints arrive later, the landing KPI endpoint can enrich matching session rows without allowing `(unknown)` values to overwrite known buckets.

`wp_kiwi_landing_funnel_daily_summary` is the persistent target model for daily landing-funnel analytics. It is populated by `Kiwi_Landing_Funnel_Daily_Summary_Aggregation_Service` from the same normalized internal tables, not from provider payloads. The main summary groups by `metric_date`, landing/service/provider/flow/country/`pid`/`tksource`, normalized `device_brand`/`os`/`os_version`/`browser` buckets, and coarse client-IP dimensions. `tkzone` is deliberately outside the main summary and belongs to `wp_kiwi_landing_funnel_daily_tkzone_summary` or legacy/debug views. Session facts derive service/provider/flow/country/source/device dimensions and stored `client_ip_version`/`client_ip_prefix` from deduplicated `wp_kiwi_landing_page_sessions`; engagement and handoff tables contribute metrics by joining directly to those canonical sessions but no longer create fallback summary sessions or repair missing dimensions in SQL. Handoff carryover scans the target day plus the next day. A direct handoff join keeps events at or after the canonical first landing, and a later-landing anti-join rejects cross-midnight session-token reuse when a newer landing day should own the event. Same-day repeated landing rows remain in the same daily bucket because the summary grain is `metric_date + landing_key + session_token`. Completed sales use durable `wp_kiwi_sales` snapshot columns for dimensions and are included only when `wp_kiwi_sales.attribution_metric_date` falls into the refreshed day. IPv4 is bucketed as `/24`, IPv6 as `/48`, and missing or invalid values go to `(unknown)` before summary refresh. Legacy landing sessions whose new stored IP buckets are empty or `(unknown)` are intentionally not re-derived from `remote_ip`, because those historical values may be infrastructure/proxy IPs. Session `metric_date` comes from `DATE(MIN(created_at))` on canonical landing-session rows. Sales dimensions come from durable `wp_kiwi_sales` snapshot columns and do not depend on temporary click-attribution rows.
...
- `dimension_hash` is based only on `landing_key`, `service_key`, `provider_key`, `flow_key`, `country`, `pid`, `tksource`, `device_brand`, `os`, `os_version`, `browser`, `client_ip_version`, and `client_ip_prefix`
- `sessions` counts distinct `landing_key + session_token` from canonical landing-session rows only
...
- sales without `attribution_metric_date` are excluded from the main daily summary until their snapshot data is repaired or backfilled
...
- handoff facts use the target day plus next-day carryover, join directly to canonical landing-session facts for the target `metric_date`, and use a later-landing anti-join so cross-midnight reused tokens are counted only for the newer landing day
- the TK-zone companion summary stores the configured PID allow-list hash on refreshed rows; read paths and filter options require the current hash so rows generated for older PID sets stay out of current TK-zone reports
- schema migration consolidates existing main-summary rows onto the slim hash/dimension contract before dropping retired columns; if consolidation fails, retired columns are kept so the migration can be retried without losing the legacy split basis

WP-Cron runs the bounded refresh contract hourly through split hooks: `kiwi_landing_funnel_daily_main_summary_refresh` for the main summary and `kiwi_landing_funnel_daily_tkzone_summary_refresh` for the TK-zone companion summary. Bootstrap clears the legacy combined `kiwi_landing_funnel_daily_summary_refresh` hook so one slow or failed summary cannot block the other. The rolling window defaults to seven lookback days plus today via `KIWI_LANDING_FUNNEL_SUMMARY_REFRESH_DAYS`; each cron invocation selects one metric date from that window, uses a summary-specific transient lock, and stores the latest non-lock result in either `kiwi_landing_funnel_daily_main_summary_refresh_last_result` or `kiwi_landing_funnel_daily_tkzone_summary_refresh_last_result`. Lock-skip diagnostics are stored separately in `kiwi_landing_funnel_daily_main_summary_refresh_lock_skip_last_result` and `kiwi_landing_funnel_daily_tkzone_summary_refresh_lock_skip_last_result` so concurrent or stale locks do not overwrite the refresh cursor. Automatic hourly refreshes use an effective one-day minimum even when the configured lookback is `0`, because cross-midnight hidden/success events after midnight can still belong to yesterday's landing-session bucket. The stored non-lock result keeps the processed `metric_date`, rolling-window bounds, `deleted`/`inserted` counts, and any compact `daily_results` entries with date+step errors such as `2026-05-23 insert aggregate rows: ...` when a chunk fails.

The refresh schema includes composite indexes for production-sized day chunks:

- `wp_kiwi_landing_page_sessions(created_at, landing_key, session_token)`
- `wp_kiwi_premium_sms_landing_engagements(created_at, landing_key, session_token)`
- `wp_kiwi_landing_handoff_events(created_at, landing_key, session_token, event_type)`
- `wp_kiwi_sales(status, attribution_metric_date)` for the main summary sales facts; `completed_at` remains relevant to legacy/debug completed-sales views
...
- forwarded request headers are trusted only when `REMOTE_ADDR` matches explicit trusted proxy CIDR configuration; otherwise the direct peer is used and spoofed headers are ignored
- sales snapshots copy stored landing-session IP buckets only; they do not reconstruct buckets from legacy `remote_ip` when `client_ip_version` or `client_ip_prefix` is missing or `(unknown)`
