...
`device_brand`, `android_version`, and `browser` are computed in the view from raw UA fields for session rows and are also persisted on new sale snapshots for durable sale analysis. `device_brand` is only populated from known manufacturer rules such as Samsung, Huawei, Google Pixel, and Xiaomi-family models; unknown model strings are grouped as `(unknown)` instead of being promoted into new brands. The view exposes `landing_key`, `service_key`, `tksource`, `tkzone`, those device/browser dimensions, session/load/CTA counters, handoff attempts/successes/fails/rate, hidden-time aggregates, and completed sales.

`wp_kiwi_landing_page_sessions` is the canonical source for server-known session dimensions. The landing-page router writes `provider_key`, `flow_key`, campaign/service `country`, `pid`, `tksource`, `tkzone`, and normalized primary `browser_language` on the first server request. Source parameters are stored even when no `click_id` is present. `browser_language` stores only the primary language tag such as `fr`, `de`, `en`, or `ar`; empty or invalid headers use `(unknown)`.

`wp_kiwi_landing_funnel_daily_summary` is the persistent target model for daily landing-funnel analytics. It is populated by `Kiwi_Landing_Funnel_Daily_Summary_Aggregation_Service` from the same normalized internal tables, not from provider payloads. The summary groups by `metric_date`, landing/service/provider/flow/country/source dimensions, normalized device/browser buckets, and coarse client-IP dimensions. Session facts derive provider/flow/country/source dimensions and `client_ip_version`/`client_ip_prefix` from `wp_kiwi_landing_page_sessions`; engagement and handoff tables contribute metrics and UA context but no longer repair those canonical session dimensions. Completed sales use the durable `wp_kiwi_sales.client_ip_version` and `wp_kiwi_sales.client_ip_prefix` snapshot columns. IPv4 is bucketed as `/24`, IPv6 as `/48`, and missing or invalid values go to `(unknown)`. Session `metric_date` remains tied to the first observed session/traffic timestamp for the current refresh model, with landing-session rows providing the stable `DATE(MIN(created_at))` source for canonical session facts. Completed sales use `wp_kiwi_sales.attribution_metric_date` with `DATE(completed_at)` only as a fallback. Sales dimensions come from durable `wp_kiwi_sales` snapshot columns and do not depend on temporary click-attribution rows.
