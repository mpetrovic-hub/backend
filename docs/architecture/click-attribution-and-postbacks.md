...
`wp_kiwi_landing_page_sessions` is the canonical source for server-known session dimensions. The landing-page router writes `provider_key`, `flow_key`, campaign/service `country`, `pid`, `tksource`, `tkzone`, normalized primary `browser_language`, and normalized device buckets (`device_brand`, `os`, `os_version`, `browser`) on the first server request. Source parameters are stored even when no `click_id` is present. `browser_language` stores only the primary language tag such as `fr`, `de`, `en`, or `ar`; empty or invalid headers use `(unknown)`. When `page_loaded` UA Client Hints arrive later, the landing KPI endpoint can enrich matching session rows without allowing `(unknown)` values to overwrite known buckets.

`wp_kiwi_landing_funnel_daily_summary` is the persistent target model for daily landing-funnel analytics. It is populated by `Kiwi_Landing_Funnel_Daily_Summary_Aggregation_Service` from the same normalized internal tables, not from provider payloads. The summary groups by `metric_date`, landing/service/provider/flow/country/source dimensions, normalized `device_brand`/`os`/`os_version`/`browser` buckets, and coarse client-IP dimensions. Session facts derive provider/flow/country/source/device dimensions and `client_ip_version`/`client_ip_prefix` from `wp_kiwi_landing_page_sessions`; engagement and handoff tables contribute metrics but no longer parse or repair device/source dimensions in SQL. Completed sales use durable `wp_kiwi_sales` snapshot columns for device and IP dimensions. IPv4 is bucketed as `/24`, IPv6 as `/48`, and missing or invalid values go to `(unknown)`. Session `metric_date` remains tied to the first observed session/traffic timestamp for the current refresh model, with landing-session rows providing the stable `DATE(MIN(created_at))` source for canonical session facts. Completed sales use `wp_kiwi_sales.attribution_metric_date` with `DATE(completed_at)` only as a fallback. Sales dimensions come from durable `wp_kiwi_sales` snapshot columns and do not depend on temporary click-attribution rows.
