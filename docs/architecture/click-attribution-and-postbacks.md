...
`wp_kiwi_landing_funnel_daily_summary` is the persistent target model for daily landing-funnel analytics. It is populated by `Kiwi_Landing_Funnel_Daily_Summary_Aggregation_Service` from the same normalized internal tables, not from provider payloads. The summary groups by `metric_date`, landing/service/provider/flow/country/source dimensions, normalized `device_brand`/`os`/`os_version`/`browser` buckets, and coarse client-IP dimensions. Session facts derive provider/flow/country/source/device dimensions and stored `client_ip_version`/`client_ip_prefix` from `wp_kiwi_landing_page_sessions`; engagement and handoff tables contribute metrics but no longer parse or repair device/source dimensions in SQL. Completed sales use durable `wp_kiwi_sales` snapshot columns for device and IP dimensions. IPv4 is bucketed as `/24`, IPv6 as `/48`, and missing or invalid values go to `(unknown)` before summary refresh. Legacy landing sessions whose new stored IP buckets are empty or `(unknown)` are intentionally not re-derived from `remote_ip`, because those historical values may be infrastructure/proxy IPs. Session `metric_date` remains tied to the first observed session/traffic timestamp for the current refresh model, with landing-session rows providing the stable `DATE(MIN(created_at))` source for canonical session facts. Completed sales use `wp_kiwi_sales.attribution_metric_date` with `DATE(completed_at)` only as a fallback. Sales dimensions come from durable `wp_kiwi_sales` snapshot columns and do not depend on temporary click-attribution rows.
...
- existing summary rows outside refreshed windows keep their previous dimension source and dbDelta defaults for newer columns until their day is recomputed; no automatic historical full backfill is implied
...
- `wp_kiwi_sales(status, attribution_metric_date)` and `wp_kiwi_sales(status, completed_at)`
