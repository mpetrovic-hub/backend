...
- handoff facts use the target day plus next-day carryover, assign each event to the latest matching landing-session row at or before the handoff event, and only join to canonical landing-session facts for the target `metric_date`
