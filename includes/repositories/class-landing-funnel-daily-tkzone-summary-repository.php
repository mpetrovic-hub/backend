...
class Kiwi_Landing_Funnel_Daily_Tkzone_Summary_Repository implements Kiwi_Statistics_Read_Repository_Interface
{
    ...
    public function get_table_name(): string
    {
        ...
        return $wpdb->prefix . 'kiwi_landing_funnel_daily_tkzone_summary';
    }
    ...
    public function delete_metric_date(string $metric_date): int
    {
        ...
        return $this->run_prepared_query(
            ...
            [$metric_date],
            'delete tkzone daily summary metric date'
        );
    }

    public function insert_aggregate_rows(string $sql, array $params): int
    {
        return $this->run_prepared_query($sql, $params, 'insert tkzone daily summary aggregate rows');
    }
    ...
    public function get_rows(array $filters = [], int $limit = 100): array
    {
        ...
        $sql = "SELECT
                ...
                FROM {$table_name}
                WHERE {$where_clause}
                ORDER BY metric_date DESC, sessions DESC, sales DESC, provider_key ASC, flow_key ASC, country ASC, service_key ASC, landing_key ASC, tksource ASC, tkzone ASC
                LIMIT %d";
    ...
    private function run_prepared_query(string $sql, array $params, string $context): int
    {
        ...
        if (!is_string($statement) && !is_array($statement)) {
            if (!$this->has_database_error()) {
                $this->last_error = 'Landing funnel daily tkzone summary ' . $context . ' prepare failed without database error detail.';
            }
        ...
        if ($result === false) {
            if (!$this->has_database_error()) {
                $this->last_error = 'Landing funnel daily tkzone summary ' . $context . ' query failed without database error detail.';
            }
    ...
    private function normalize_filter_value(string $value, string $field = ''): string
    {
        ...
        if (in_array($field, ['provider_key', 'flow_key'], true)) {
        ...
        if (in_array($field, ['service_key', 'landing_key'], true)) {
